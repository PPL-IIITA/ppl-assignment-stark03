...
logging.basicConfig(format='%(asctime)s %(name)-6s %(levelname) s: %(message)s',datefmt='%d/%m/%Y %I:%M:%S %p',level=logging.DEBUG,filename='logBreakUpPatchUp.txt', filemode='w')
    ...
    def breakLeastHappy(self,k):
        ...
        if((k <= Couples.__len__())):
            for i in range (0,k):
                ...
                logging.info("%s broke up with %s" % (Couples[i].gName , Couples[i].bName ))
                    ...
                    if (criteria == 'mostAttractive'):
                        ...
                        for t in arrBoys:
                            if (t.minimumAttrReq >= arrGirls[i].attractiveness and t.budget >= arrGirls[
                                ...
                                logging.info("%s recently patched up with %s , her old boyfriend was %s" % (arrGirls[i].name , t.name , arrGirls[i].blackList))
                                temp = couple(t, arrGirls[i])
                                temp.happinessCalcuator(t, arrGirls[i])
                                temp.compatibilityCalculator(t, arrGirls[i])
                                Couples.append(temp)
                        ...
                        for k in arrBoys:
                            if (k.minimumAttrReq >= arrGirls[i].attractiveness and k.budget > arrGirls[
                                ...
                                logging.info("%s recently patched up with %s , her old boyfriend was %s" % (arrGirls[i].name , k.name , arrGirls[i].blackList))
                                temp = couple(k, arrGirls[i])
                                temp.happinessCalcuator(k, arrGirls[i])
                                temp.compatibilityCalculator(k, arrGirls[i])
                                Couples.append(temp)
                        ...
                        for y in arrBoys:
                            if (y.minimumAttrReq >= arrGirls[i].attractiveness and arrGirls[
                                ...
                                logging.info("%s recently patched up with %s , her old boyfriend was %s" % (arrGirls[i].name , y.name , arrGirls[i].blackList))
                                temp = couple(y, arrGirls[i])
                                temp.happinessCalcuator(y, arrGirls[i])
                                temp.compatibilityCalculator(y, arrGirls[i])
                                Couples.append(temp)
