...
class boyGenerous(Boy):
    ...
    def gifting(self, gMaintainanceCost, Gifts, giftBasket , y):
        ...
        "Sets up the gift basket for Generous boys with best k gifts sorted on basis of value of the gift"

        Gifts.sort(key=operator.attrgetter('value'), reverse=True)
        ...
        if self.budget < Gifts[0].price:
            ...
            i = 0
            while(True):
                if(self.amountSpent < self.budget and i < y):
