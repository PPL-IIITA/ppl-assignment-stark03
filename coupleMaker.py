...
class coupleMaker:
    ...
    def makeCouples(self):
        """Make Couples and print the gifting details of each"""
        r = read()
        r.readcsvfile('b')
        r.readcsvfile('g')
        r.readcsvfile('f')
    ...
    def getMostCompatible(self, k):
        """Prints the k most compatible couples"""
        Couples.sort(key=operator.attrgetter('compatibility'),reverse=True)
        if (k <= Couples.__len__()):
            for i in range(0, k):
                print("Boy :- %s , Girl :- %s , Compatibility :- %d" % (Couples[i].bName,Couples[i].gName , Couples[i].compatibility))
        else:
            print("k is greater than the no. of couples formed")
